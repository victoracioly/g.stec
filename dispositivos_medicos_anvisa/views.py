...
def build_q_object(field_name, terms):
    """
    Constrói um Q object para buscar diversos termos em um mesmo campo,
    usando __icontains para cada termo e unindo com OR.
    """
    q = Q()
    for term in terms:
        t = term.strip()
...
def lista_dispositivos(request):
    ...
    # Captura filtros de texto do GET
    ...
    vigentes        = request.GET.get('vigentes', '').strip()
    ...
    # Filtro "Somente Vigentes"
    filtros = []
    if vigentes == '1':
        hoje_iso = date.today().isoformat()  # 'YYYY-MM-DD'
        pattern = r'(?i)\b(vigente|vigentes|val[ií]do|val[ií]dos)\b'
        qs = qs.filter(
            Q(validade_registro__gte=hoje_iso) |
            Q(validade_registro__iregex=pattern)
        )
        filtros.append("Somente Vigentes")

    total_dispositivos = qs.count()

    # Estatísticas de país e fabricante
    top_paises = (
    ...
    # Ordena: datas primeiro, depois textos
    qs = qs.annotate(
        is_date=Case(
    ...
    # Prepara atributo validade_display (texto bruto ou fallback 'VIGENTE')
...
#     filtros = []
#
#     # Aplicar mesmos filtros de texto
#     campos = [
...
#         if valor:
#             qs = qs.filter(build_q_object(param, valor.split(',')))
#             filtros.append(f"{label}: {', '.join(valor.split(','))}")
#
#     # Somente Vigentes no PDF
